...
module.exports = Player = Character.extend({
    ...
    setTarget: function(entity) {
        if (this.hasTarget()) {
            var target = this.server.getEntityById(this.target);
            if (target instanceof Character) {
              target.removeTargeterBinding(this.id);
            }
        }
        this._super(entity);
        if (entity instanceof Character) {
          entity.addTargeterBinding(this.id);
          this.send(new Messages.TargetHealth(entity).serialize());
        }
    }
});
